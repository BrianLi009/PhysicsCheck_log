...
for i in $(seq 1 $new_index) #1-based indexing for cubes
    do
        ...
        command4="if ! grep -q 'UNSATISFIABLE' '$d/$v/$n-solve/$i-solve.log'; then sbatch $child_instance-solve.sh; fi"
        #sbatch this line
        ...
        echo "#SBATCH --time=0-08:00" >> $child_instance-solve.sh
        echo "#SBATCH --mem-per-cpu=2G" >> $child_instance-solve.sh
        echo "#!/bin/bash" > $child_instance-cube.sh
        echo "#SBATCH --account=def-vganesh" >> $child_instance-cube.sh
        echo "#SBATCH --time=0-08:00" >> $child_instance-cube.sh
        echo "#SBATCH --mem-per-cpu=2G" >> $child_instance-cube.sh
